...
$query = $dbh->prepare("SELECT categorie FROM categorie_Personal where id = :categories");
$query->execute(
    [
...
uploadPhotos("personal", $photoCat["categorie"]);

if (isset($_POST["id"]))// UPDATE
{
    $query = $dbh->prepare(
        "SELECT categorie FROM personal INNER JOIN categorie_Personal ON personal.id_Categorie = categorie_Personal.id WHERE personal.id = :id"
    );
    $query->execute(
        [
            "id" => $_POST["id"],
    ...
    {
        $query = $dbh->prepare(
            "UPDATE personal INNER JOIN categorie_Personal ON personal.id_Categorie = categorie_Personal.id SET titre = :titre, classement = :classement, id_Categorie = :id_Categorie WHERE personal.id = :id"
        );
        $query->execute(
            [
    ...
    if ($oldCat["categorie"] != $newCat["categorie"]) {
        copy(
            "../../../assets/images/personal/".$oldCat["categorie"]."/".$newCat["photo"],
            "../../../assets/images/personal/".$newCat["categorie"]."/".$newCat["photo"]
...
} else//INSERT
{
    ...
    $query = $dbh->prepare(
        "INSERT INTO personal (id_Categorie,photo,titre,classement) Values (:id_Categorie, :photo, :titre, :classement)"
    ); //INSERTION
    $query->execute(
        [
            "id_Categorie" => $_POST["categories"],
