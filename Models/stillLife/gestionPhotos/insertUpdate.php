...
$query = $dbh->prepare("SELECT categorie FROM categorie_StillLife where id = :categories");
$query->execute(
    [
...
uploadPhotos("stillLife", $photoCat["categorie"]);

if (isset($_POST["id"]))// UPDATE
{
    $query = $dbh->prepare(
        "SELECT categorie FROM stillLife INNER JOIN categorie_StillLife ON personal.id_Categorie = categorie_StillLife.id WHERE stillLife.id = :id"
    );
    $query->execute(
        [
            "id" => $_POST["id"],
    ...
    {
        $query = $dbh->prepare(
            "UPDATE stillLife INNER JOIN categorie_StillLife ON stillLife.id_Categorie = categorie_StillLife.id SET titre = :titre, classement = :classement, id_Categorie = :id_Categorie WHERE stillLife.id = :id"
        );
        $query->execute(
            [
    ...
    if ($oldCat["categorie"] != $newCat["categorie"]) {
        copy(
            "../../../assets/images/stillLife/".$oldCat["categorie"]."/".$newCat["photo"],
            "../../../assets/images/stillLife/".$newCat["categorie"]."/".$newCat["photo"]
...
} else//INSERT
{
    ...
    $query = $dbh->prepare(
        "INSERT INTO stillLife (id_Categorie,photo,titre,classement) Values (:id_Categorie, :photo, :titre, :classement)"
    ); //INSERTION
    $query->execute(
        [
            "id_Categorie" => $_POST["categories"],
